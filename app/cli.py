...
from app.services.trending import trending_engine, recommendation_engine, populate_trending_from_db
...
def trending_cmd(
    window: int = typer.Option(60, "--window", "-w", help="Time window in minutes (1-1440)", min=1, max=1440),
    k: int = typer.Option(10, "--k", "-k", help="Number of top hashtags to return (1-100)", min=1, max=100),
    sync: bool = typer.Option(False, "--sync", help="Sync with database before showing trending"),
    sync_minutes: int = typer.Option(60, "--sync-minutes", help="Minutes of data to sync from database", min=1, max=1440),
):
...
def recommend_cmd(
    ...
    max_recommendations: int = typer.Option(3, "--max", "-m", help="Maximum recommendations (1-10)", min=1, max=10),
):

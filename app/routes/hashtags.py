...
from app.services.trending import recommendation_engine, trending_engine
...
class RecommendationResponse(BaseModel):
    ...
    recommendations: List[RecommendedHashtag] = Field(
        ..., description="List of recommended hashtags"
    )
...
async def get_trending_hashtags(
    ...
    try:
        ...
        hashtags = [
            TrendingHashtag(hashtag=hashtag, count=count) for hashtag, count in trending_data
        ]

        return TrendingResponse(hashtags=hashtags, window_minutes=window, total_count=len(hashtags))

    except Exception as e:
        raise HTTPException(
            status_code=500, detail=f"Failed to retrieve trending hashtags: {str(e)}"
        )
...
async def get_engine_status() -> EngineStatus:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to retrieve engine status: {str(e)}")
...
async def sync_from_database(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to sync trending data: {str(e)}")

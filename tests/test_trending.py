...
from app.models import Base, Hashtag, Post, PostHashtag, User
from app.services.trending import RecommendationEngine, TrendingEngine, populate_trending_from_db
...
def sample_data(in_memory_db):
    ...
    try:
        ...
        posts = [
            Post(
                user_id=users[0].id,
                content="Python is great #python #fastapi",
                created_at=now - timedelta(minutes=5),
            ),
            Post(
                user_id=users[1].id,
                content="Testing with FastAPI #fastapi #testing",
                created_at=now - timedelta(minutes=3),
            ),
            Post(
                user_id=users[2].id,
                content="AI and ML #ai #ml #python",
                created_at=now - timedelta(minutes=1),
            ),
            Post(
                user_id=users[0].id,
                content="More Python content #python",
                created_at=now - timedelta(minutes=2),
            ),
            Post(
                user_id=users[1].id,
                content="FastAPI rocks #fastapi #python",
